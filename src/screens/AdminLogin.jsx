  ...
  return (
    <div style={{
      ...
      <style>{`
        ...
        .ts-input-d:focus{outline:none;border-color:#ef4444;box-shadow:0 0 0 3px rgba(220,38,38,.22)}
      ...
      <div className="ts-admin-left" style={{
        flex: 1.1,
        background: 'radial-gradient(120% 60% at 10% 0%,rgba(220,38,38,.38) 0%,transparent 55%),radial-gradient(80% 50% at 100% 100%,rgba(16,185,129,.2) 0%,transparent 65%),linear-gradient(160deg,#0d1528 0%,#111827 100%)',
        padding: '36px 56px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between',
        position: 'relative', overflow: 'hidden', minWidth: 0,
      }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, zIndex: 5, position: 'relative' }}>
        ...
        <div style={{ fontSize: 11, color: '#475569', zIndex: 5, position: 'relative' }}>
          © TingSync · <span style={{ color: '#f87171' }}>Attendance Policy</span>
        </div>

        {/* Rotating clock decoration */}
        <svg viewBox="0 0 400 400" fill="none" style={{ position: 'absolute', right: -80, bottom: -80, width: 520, height: 520, pointerEvents: 'none', filter: 'drop-shadow(0 0 40px rgba(220,38,38,.22))', zIndex: 1, opacity: .7 }}>
          <defs>
          ...
          <g stroke="rgba(239,68,68,.28)" strokeWidth="2" strokeLinecap="round">
          ...
          <g className="ts-hand-m-d"><rect x="198.5" y="90" width="3" height="120" rx="1.5" fill="rgba(239,68,68,.55)"/></g>
        ...
        <div style={{ maxWidth: 460, width: '100%', margin: '0 auto' }}>
          ...
          <form onSubmit={submit}>
            <div style={{ marginBottom: 14 }}>
              ...
              <input className="ts-input-d" type="text" value={companyCode} onChange={(e) => setCompanyCode(e.target.value.toUpperCase())} placeholder="TINGSYNC-2026"
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ position: 'relative' }}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#ef4444" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ position: 'absolute', left: 14, top: 15 }}><circle cx="12" cy="8" r="4"/><path d="M4 21c0-4 4-6 8-6s8 2 8 6"/></svg>
                <input className="ts-input-d" type="text" value={username} onChange={(e) => setUsername(e.target.value)} placeholder="admin"
                  autoCapitalize="none" autoCorrect="off"
                  style={{ width: '100%', boxSizing: 'border-box', border: '1.5px solid rgba(220,38,38,.28)', background: 'rgba(255,255,255,.04)', borderRadius: 12, padding: '13px 16px 13px 40px', color: '#e2e8f0', fontSize: 14, fontWeight: 600, fontFamily: 'inherit' }} />
              </div>
            </div>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ position: 'relative' }}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#ef4444" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ position: 'absolute', left: 14, top: 15 }}><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg>
                <input className="ts-input-d" type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••••"
                  style={{ width: '100%', boxSizing: 'border-box', border: '1.5px solid rgba(220,38,38,.28)', background: 'rgba(255,255,255,.04)', borderRadius: 12, padding: '13px 16px 13px 40px', color: '#e2e8f0', fontSize: 16, fontWeight: 700, fontFamily: 'inherit', letterSpacing: 4 }} />
              </div>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', margin: '16px 0 22px' }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: '#94a3b8', fontWeight: 600, cursor: 'pointer' }}>
                <span onClick={() => setRemember(!remember)} style={{ width: 18, height: 18, borderRadius: 6, background: remember ? 'linear-gradient(135deg,#dc2626,#f97316)' : 'rgba(255,255,255,.04)', border: remember ? 'none' : '1px solid rgba(255,255,255,.15)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', boxShadow: remember ? '0 2px 8px rgba(220,38,38,.4)' : 'none' }}>
            ...
            <button type="submit" disabled={busy} className="ts-btn-primary-d" style={{
              width: '100%', background: 'linear-gradient(135deg,#dc2626,#f97316)', color: '#fff', border: 'none',
              borderRadius: 14, padding: 16, fontWeight: 800, fontSize: 15, fontFamily: 'inherit',
              cursor: busy ? 'not-allowed' : 'pointer',
              boxShadow: '0 8px 24px rgba(220,38,38,.4), inset 0 1px 0 rgba(255,255,255,.15)',
          ...
          <div style={{ textAlign: 'center', fontSize: 11, color: '#475569', marginTop: 20 }}>
            By signing in you agree to the <span style={{ color: '#f87171', fontWeight: 700 }}>Attendance Policy</span> and <span style={{ color: '#f87171', fontWeight: 700 }}>Privacy Notice</span>
          </div>

          <Link to="/employee" style={{
            display: 'block', textAlign: 'center', marginTop: 20, textDecoration: 'none',
            fontSize: 12, fontWeight: 800, letterSpacing: 1.4, color: '#f87171',
          }}>

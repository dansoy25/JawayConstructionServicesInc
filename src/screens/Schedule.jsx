...
function LeaveSection({ dark, cardBg, cardBorder, textPrimary, textMuted }) {
  ...
  const submit = async (e) => {
    ...
    setBusy(true); setMsg('')
    try {
      // If admin hasn't set up any leave types, create a default "Personal Leave"
      // so the request still lands — the employee shouldn't be blocked by admin config.
      ...
      // Surface the raw supabase error so we can see what's actually wrong.
      setMsg(`Leave request failed: ${e.message || e}`)
    }
    setBusy(false)
  }

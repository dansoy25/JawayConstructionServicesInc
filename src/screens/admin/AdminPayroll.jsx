...
export default function AdminPayroll() {
  ...
  const deleteRun = async (r) => {
    if (!confirm(`Delete payroll run for ${fmtDate(r.period_start)} — ${fmtDate(r.period_end)}?\nAll payslips generated by this run will also be deleted.`)) return
    // Delete matching payslips first (they don't have an FK cascade in-place).
    await supabase.from('payslips').delete().eq('org_id', r.org_id)
      .eq('period_start', r.period_start).eq('period_end', r.period_end)
    ...
    <div style={{ padding: 32 }}>
      ...
      <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 14, overflow: 'hidden' }}>
        ...
        <table style={table}>
          ...
          <tbody>
            {runs.length === 0 && <tr><td colSpan="8" style={{ ...td, textAlign: 'center', padding: 30, color: '#94a3b8' }}>No payroll runs yet. Click "Run payroll" to get started.</td></tr>}
            {runs.map((r) => (
              <tr key={r.id}>
                ...
                <td style={td}>
                  <button
                    onClick={() => deleteRun(r)}
                    title="Delete this payroll run and its payslips"
